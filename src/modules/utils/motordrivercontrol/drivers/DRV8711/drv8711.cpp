...
bool DRV8711DRV::check_alarm()
{
    ...
    if(R_STATUS_REG.OTS) {
        if(!error_reported.test(0)) THEKERNEL->report_error(false, 126, "DRV8711: Overtemperature shutdown, designator: ", "%c", designator);
        error= true;
    ...
    if(R_STATUS_REG.AOCP) {
        if(!error_reported.test(1)) THEKERNEL->report_error(false, 127, "DRV8711: Channel A over current shutdown, designator: ", "%c", designator);
        error= true;
    ...
    if(R_STATUS_REG.BOCP) {
        if(!error_reported.test(2)) THEKERNEL->report_error(false, 128,  "DRV8711: Channel B over current shutdown, designator: ", "%c", designator);
        error= true;
    ...
    if(R_STATUS_REG.APDF) {
        if(!error_reported.test(3)) THEKERNEL->report_error(false, 129,  "DRV8711: Channel A predriver fault, designator: ", "%c", designator);
        error= true;
    ...
    if(R_STATUS_REG.BPDF) {
        if(!error_reported.test(4)) THEKERNEL->report_error(false, 130,  "DRV8711: Channel B predriver fault: ", "%c", designator);
        error= true;
