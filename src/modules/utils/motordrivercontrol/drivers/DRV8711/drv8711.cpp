...
bool DRV8711DRV::check_alarm()
{
    ...
    if(R_STATUS_REG.OTS) {
        if(!error_reported.test(0)) THEKERNEL->report_error(false, 126, "%c", designator); //THEKERNEL->streams->printf("%c, ERROR: Overtemperature shutdown\n", designator);
        error= true;
    ...
    if(R_STATUS_REG.AOCP) {
        if(!error_reported.test(1)) THEKERNEL->report_error(false, 127, "%c", designator); //THEKERNEL->streams->printf("%c, ERROR: Channel A over current shutdown\n", designator);
        error= true;
    ...
    if(R_STATUS_REG.BOCP) {
        if(!error_reported.test(2)) THEKERNEL->report_error(false, 128, "%c", designator); //THEKERNEL->streams->printf("%c, ERROR: Channel B over current shutdown\n", designator);
        error= true;
    ...
    if(R_STATUS_REG.APDF) {
        if(!error_reported.test(3)) THEKERNEL->report_error(false, 129, "%c", designator); //THEKERNEL->streams->printf("%c, ERROR: Channel A predriver fault\n", designator);
        error= true;
    ...
    if(R_STATUS_REG.BPDF) {
        if(!error_reported.test(4)) THEKERNEL->report_error(false, 130, "%c", designator); //THEKERNEL->streams->printf("%c, ERROR: Channel B predriver fault\n", designator);
        error= true;
