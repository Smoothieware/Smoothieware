...
void Panel::on_module_loaded()
{
    ...
    this->up_button.up_attach(    std::bind(&Panel::on_up, this) );
    this->down_button.up_attach(  std::bind(&Panel::on_down, this) );
    this->click_button.up_attach( std::bind(&Panel::on_select,this) );
    this->back_button.up_attach(  std::bind(&Panel::on_back,this) );
...
//    this->pause_button.set_longpress_delay(longpress_delay);
    ...
    THEKERNEL->slow_ticker->attach( 50,  std::bind(&Panel::button_tick, this) );
    if(lcd->encoderReturnsDelta()) {
        // panel handles encoder pins and returns a delta
      THEKERNEL->slow_ticker->attach( 10, std::bind(&Panel::encoder_tick,this) );
    }else{
        // read encoder pins
      THEKERNEL->slow_ticker->attach( 1000, std::bind( &Panel::encoder_check, this) );
    }
    ...
    // Refresh timer
    THEKERNEL->slow_ticker->attach( 20, std::bind( &Panel::refresh_tick,this) );
}
