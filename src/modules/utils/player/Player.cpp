...
void Player::on_suspend(void *argument)
{
    ...
    // clean up from suspend
        ...
        this->was_playing_file = true;
        playing_file = false;
        this->suspend_loops = 1;
        THEKERNEL->streams->printf("//test suspend!!!\n");
...
void Player::on_console_line_received(void *argument)
{
    ...
    if (cmd == "play")
    {
        if (suspended)
        {
            new_message.stream->printf("suspended!\n");
            return;
        }
    ...
    {
        this->suspend_command_brm(possible_command, new_message.stream);
    }
    else if (cmd == "resume")
    {
...
void Player::on_main_loop(void *argument)
{
    ...
    if (this->playing_file)
    {
        ...
        while (fgets(buf, sizeof(buf), this->current_file_handler) != NULL)
        {
            int len = strlen(buf);
            if (len == 0)
                continue; // empty line? should not be possible
            if (buf[len - 1] == '\n' || feof(this->current_file_handler))
            {
                ...
                std::copy(buf,buf+130,resume_buf);
...
void Player::on_get_public_data(void *argument)
{
    ...
    if (!pdr->starts_with(player_checksum))
        return;
...
*/
void Player::suspend_command_brm(string parameters, StreamOutput *stream)
{
