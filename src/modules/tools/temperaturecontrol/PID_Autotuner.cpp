...
void PID_Autotuner::on_idle(void *)
{
    ...
    if(peakCount >= requested_cycles) {
        ...
        THEKERNEL->report_error(false, 27, "AutoPID didn't resolve in enough cycles, cylcles: #%d", requested_cycles);    // THEKERNEL->streams->printf("// WARNING: Autopid did not resolve within %d cycles, these results are probably innacurate\n", requested_cycles);
        finishUp();
        return;
    }
