...
void TemperatureControl::on_gcode_received(void* argument){
    ...
    if (gcode->has_m)
    {
        ...
        if( ( gcode->m == this->set_m_code || gcode->m == this->set_and_wait_m_code ) && gcode->has_letter('S') ){
            ...
            Block* block = kernel->conveyor->next_block();

            TemperatureControlData* data = new TemperatureControlData();

            data->target_temperature = v;
            data->wait = (gcode->m == this->set_and_wait_m_code);
...
void TemperatureControl::on_block_invoke(void* argument)
{
    TemperatureControlData* data = static_cast<TemperatureControlData*>(argument);

    double v = data->target_temperature;
