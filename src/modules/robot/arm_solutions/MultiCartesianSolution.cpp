...
//So a conventional cartesion is 2103, and our default for 6 motors is 2102106.
//You should have at least one motor per axis.

#define axis_map_checksum CHECKSUM("multicartesion.axis_map")

MultiCartesianSolution::MultiCartesianSolution(Config* config)
{
    int axis_map =  std::max(config->value(axis_map_checksum)->by_default(2102106)->as_int(), 0);
