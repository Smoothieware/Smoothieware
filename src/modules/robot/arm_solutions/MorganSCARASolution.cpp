...
MorganSCARASolution::MorganSCARASolution(Config* config)
{
    ...
    arm1_length         = config->value((unsigned int)arm1_length_checksum)->by_default(150.0f)->as_number();
    // arm2_length is the length of the inner main arm from hinge to hinge
    arm2_length         = config->value((unsigned int)arm2_length_checksum)->by_default(150.0f)->as_number();
    // morgan_offset_x is the x offset of bed zero position towards the SCARA tower center
    morgan_offset_x     = config->value((unsigned int)morgan_offset_x_checksum)->by_default(100.0f)->as_number();
    // morgan_offset_y is the y offset of bed zero position towards the SCARA tower center
    morgan_offset_y     = config->value((unsigned int)morgan_offset_y_checksum)->by_default(-60.0f)->as_number();
    ...
    morgan_undefined_min  = config->value((unsigned int)morgan_undefined_min_checksum)->by_default(0.95f)->as_number();
    // max: head on maximum reach
    morgan_undefined_max  = config->value((unsigned int)morgan_undefined_max_checksum)->by_default(0.95f)->as_number();
