...
TCP::TCP(Encapsulator* new_parent)
{
    parent = new_parent;
}

int TCP::receive( NetworkInterface* interface, NET_PACKET packet, int length)
{
    int txlength = 0;
...
int TCP::construct( NetworkInterface* interface, NET_PACKET packet, int length)
{
...
NET_PAYLOAD TCP::get_payload_buffer(NET_PACKET packet)
{
    tcp_frame* tcf = (tcp_frame*) parent->get_payload_buffer(packet);

    return tcf->payload;
}

void TCP::set_payload_length(NET_PACKET packet, int length)
{
    tcp_frame* tcf = (tcp_frame*) parent->get_payload_buffer(packet);
...
int TCP::periodical(int milliseconds, NetworkInterface* interface, NET_PACKET buffer, int bufferlen)
{
    return 0;
}

NET_PACKET TCP:: get_new_packet_buffer(NetworkInterface* ni)
{
    tcp_frame* frame = (tcp_frame*) parent->get_new_packet_buffer(ni);
    // TODO: fill in sensible defaults for a tcp packet
    bzero(frame, sizeof(tcp_frame));
    return (NET_PACKET) frame;
}

// bool TCP::add_interface(NetworkInterface* interface)
// {
...
// int TCP::receive_IPV4(NetworkInterface* interface, uint8_t* buf, uint8_t* payload, int length)
// {
//     int txlength = 0;
//
...
// int TCP::receive_packet(NetworkInterface* interface, uint8_t* buf, int length)
// {
//     int txlength = 0;
//
...
// int TCP::periodical(NetworkInterface* interface, uint8_t* buf)
// {
//     return 0;
// }
