...
static int GetPacketDataAndChecksum(char* pBuffer, size_t BufferSize, unsigned char* pChecksum, char* pLastChar)
{
...
static int AttemptToGetPacketData(char* pBuffer, size_t BufferSize, char** pStartOfPacket)
{
    char           LastChar = '\0';
    unsigned char  Checksum;
    int            GoodPacket;
    ...
    do
    {
        ...
        GoodPacket = GetPacketDataAndChecksum(pBuffer, BufferSize, &Checksum, &LastChar);
    } while (!GoodPacket);
    
    if (ValidatePacketChecksum(Checksum))
    {
        SendACKToGDB();
        *pStartOfPacket = &pBuffer[0];
        return 1;
    }
    else
    {
...
static char* _GetPacketFromGDB(char* pBuffer, size_t BufferSize)
{
    ...
    do
    {
        PacketChecksumGood = AttemptToGetPacketData(pBuffer, BufferSize, &StartOfPacket);
...
static void SMriState_GetPacketFromGDB(SMriState* pState)
{
    ...
    pStartOfPacket = _GetPacketFromGDB(pState->InputOutputBuffer, sizeof(pState->InputOutputBuffer));
    SBuffer_Init(&pState->Buffer, pStartOfPacket, strlen(pStartOfPacket));
}
...
{
    static const unsigned short SemihostBreakpointMachineCode = 0xbeab;
    const unsigned short*       pCurrentInstruction = (const unsigned short*)g_MriContext.PC;
    
    return (SemihostBreakpointMachineCode == *pCurrentInstruction);
}
...
static int HandleSemihostUidRequest(void)
{
...
static int HandleSemihostRequest(void)
{
    unsigned int OpCode;
    int          ReturnCode;
    ...
    {
    case 257:
        ReturnCode = HandleSemihostUidRequest();
        break;
    default:
        return 0;
    }
...
extern "C" void MriDebugException(void)
{
    ...
    do
    {
        StartDebuggeeUpAgain = HandleGDBCommand(&g_MriState);
